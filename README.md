...
| HQ coverage | [Usage](docs/hq-coverage-gate.md) · `scripts/hq-coverage-gate.py` | Checks current reference-feature proof receipts before the explicit guarded render route; visual judgment and owner approval stay separate |
...
| Tests | `tests/execution`, `tests/production-gate`, `tests/knowledge`, `tests/hq-coverage` | Execution-contract, production-gate, knowledge-catalog, and HQ-coverage suites |
