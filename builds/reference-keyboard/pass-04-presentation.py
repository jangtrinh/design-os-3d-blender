...
from project import ROOT,BUILD,layout,key_rows,output_context,write,sha,spec_path
import bpy
...
assert gate['inputs']['spec_sha256']==sha(spec_path()),'gate is bound to another spec'
...
for obj in bpy.context.scene.objects:
    ...
    role=('silver' if n=='RK_BASE' or n.startswith('RK_KNOB_') else
          ...
          'silver' if n.startswith(('RK_ENCODER_','RK_GUIDE_','RK_USB_TAB_')) else
...
g.assign(bpy.data.objects['TOOL_SWITCH_RING'],materials['switch'])
normal_finish=studio.finish_control_normals()
...
write(out/'provenance.json',{'source_blend':source,'source_sha256':sha(source),'form_gate':gate_path,
                            'form_gate_sha256':sha(gate_path),'declared_inputs':inputs['project'],'normal_finish':normal_finish,
