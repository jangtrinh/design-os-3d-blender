...
def chassis(data,keys):
    ...
    operation(acrylic,g.slab('CUT_PCB_SEAT',.2804,.0884,.002,.0118,.013))
    ...
    usb=g.slab('CUT_USB_WINDOW',.010,.006,.0005,.006,.010,(.067,.045))
    operation(acrylic,usb)

    pcb=g.slab('RK_PCB',.280,.088,.002,.0118,.0134)
    hole_set(pcb,mount,.0029,.011,.014,'CUT_PCB_SPACER_')
    pins=[]
    for row in keys:
        x,y=[v/1000 for v in row['xy_mm']]
        pins.extend(((x-.0035,y-.0026),(x+.0035,y-.0026)))
    hole_set(pcb,pins,.0005,.011,.014,'CUT_CONTACT_')
    ...
    hole_set(plate,knob_xy,.0044,.013,.016,'CUT_ENCODER_NECK_')
    hole_set(pcb,knob_xy,.0044,.011,.014,'CUT_PCB_ENCODER_')
