...
def run(scene_report_dict:dict,expected_layout_json)->dict:
    ...
    if housing is not None:
        ...
        unqualified("switch_housing_envelope",
                    {"array_bbox_mm":[round(v,4) for v in _dims(housing)]},
                    {"required":"per-cell evaluated geometry envelopes","nominal_mm":[float(layout["switch_width_mm"]),float(layout["switch_width_mm"]),float(layout["switch_height_mm"])]})
    ...
    return {
        ...
        "physical_scope_status":"unqualified" if unqualified_names else "checked",
        ...
        "limits":[
            ...
            "The nominal 15.6 x 15.6 x 11 mm switch envelope is not qualified by the current array report; per-cell evaluated geometry is required.",
