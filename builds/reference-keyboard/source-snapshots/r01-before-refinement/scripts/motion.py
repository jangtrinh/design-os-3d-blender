...
def setup():
    ...
    groups={
        ...
        'switches':_group('ASSEMBLY_SWITCHES',select(lambda n:n.startswith(('RK_SWITCH_','RK_CONTACT_')))),
        'pcb':_group('ASSEMBLY_PCB',select(lambda n:n=='RK_PCB' or n.startswith('RK_COMPONENT_'))),
        'diffuser':_group('ASSEMBLY_DIFFUSER',select(lambda n:n=='RK_DIFFUSER' or n.startswith('RK_USB_'))),
    ...
    for i,key in enumerate(keys):
        start=3+i%10
        animate_property_keys(key,'location',[(1,.0175),(start,.0175),(start+2,.016),(start+4,.0175),(96,.0175)],2,'LINEAR')
        stem=bpy.data.objects['RK_SWITCH_STEM_%02d'%i]
        add_simple_driver(stem,'location',2,key,'location',2,'var - 0.0175')
    ...
    return {'frames':[1,96],'fps':24,'keys':len(keys),'drivers':len(keys),'assembly_groups':list(groups),
            ...
            'limits':['Exploded views are choreography, not a validated hardware assembly procedure.',
                      'Press travel uses 1.5 mm; keycap stem engagement remains unqualified.']}
    ...
    try:
        for frame in (1,5,7,12,18,24,40,58,66,90,96):
            ...
            for i in range(58):
                ...
                travel=(.0175-key.location.z)*1000
                assert -.001<=travel<=1.501,(frame,i,travel)
                assert abs(stem.location.z-(key.location.z-.0175))<1e-7,(frame,i,'driver')
        ...
        assert max(v for row in rows for v in row['key_travel_mm'].values())>1.499
