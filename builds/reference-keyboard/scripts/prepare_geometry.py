...
from project import ROOT,BUILD,layout,key_rows,write,spec_path,contract_path,interfaces
...
raw['roles']['physical'] += ['RK_KEY_00','RK_KEY_41','RK_KNOB_1','RK_GUIDE_SLEEVE_0','RK_ENCODER_CARRIER_1']
write(contract_path(),raw)
...
spec={'schema_version':1,'units':'mm','project':'CK-001 reference reconstruction, digital geometry screening',
      ...
      'parts':[part('RK_BASE',[284,92,4],1.2,holes(1,3.4)),
               ...
               part('RK_KNOB_1',[16,16,18],1.1),part('RK_FOOT_0',[14,5,2],1.2),
               part('RK_GUIDE_SLEEVE_0',[6,6,7.9],1.0)],
...
sources=[BUILD/'layout.json',BUILD/'interfaces.json',spec_path(),contract_path()]
sources += [BUILD/'scripts'/n for n in ('project.py','meshkit.py','mechanics.py','switches.py','controls.py','supports.py','encoders.py')]
...
write(BUILD/(sys.argv[1] if len(sys.argv)>1 else 'geometry-pipeline-B.json'),{'version':1,'pipeline_id':'reference-keyboard-geometry-B','steps':[{
